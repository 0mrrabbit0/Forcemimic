...
def main(args):
    ...
    # 构建SLAM
    stop_start = input(
        "Press enter to stop closing gripper and start D455 construct map"
    )
    if stop_start != "":
        del d415, d455, bluedot_lb75, angler
        return
    ...
    shutil.copyfile(
        os.path.join(args.bdft_tare_path, "tare_bdft.json"),
        os.path.join(args.save_path, "bdft", "tare_bdft.json"),
    )
