...
def main(args):
    ...
    for d415_current_idx in tqdm.trange(len(d415_timestamps)):
        ...
        bdft_xyz_d415, bdft_quat_d415 = bdft_pose_d415[:3, 3], Rot.from_matrix(bdft_pose_d415[:3, :3]).as_quat()
        ...
        if clip_object:
            ...
            clip_object_mask = (d415_pc_xyz_base[:, 0] > OBJECT_SPACE[0][0]) & (d415_pc_xyz_base[:, 0] < OBJECT_SPACE[0][1]) & \
                                (d415_pc_xyz_base[:, 1] > OBJECT_SPACE[1][0]) & (d415_pc_xyz_base[:, 1] < OBJECT_SPACE[1][1]) & \
                                (d415_pc_xyz_base[:, 2] > OBJECT_SPACE[2][0]) & (d415_pc_xyz_base[:, 2] < OBJECT_SPACE[2][1])
        else:
        ...
        if clip_bdft:
            ...
            clip_bdft_mask = (d415_pc_xyz_bdft[:, 0] > BDFT_SPACE[0][0]) & (d415_pc_xyz_bdft[:, 0] < BDFT_SPACE[0][1]) & \
                                (d415_pc_xyz_bdft[:, 1] > BDFT_SPACE[1][0]) & (d415_pc_xyz_bdft[:, 1] < BDFT_SPACE[1][1]) & \
                                (d415_pc_xyz_bdft[:, 2] > BDFT_SPACE[2][0]) & (d415_pc_xyz_bdft[:, 2] < BDFT_SPACE[2][1])
        else:
            clip_bdft_mask = np.zeros((d415_pc_xyz_d415.shape[0],), dtype=bool)
        if clip_base:
            d415_pc_xyz_base = transform_pc(d415_pc_xyz_d415, D415_2_BASE)
            clip_base_mask = (d415_pc_xyz_base[:, 0] > BASE_SPACE[0][0]) & (d415_pc_xyz_base[:, 0] < BASE_SPACE[0][1]) & \
                                (d415_pc_xyz_base[:, 1] > BASE_SPACE[1][0]) & (d415_pc_xyz_base[:, 1] < BASE_SPACE[1][1]) & \
                                (d415_pc_xyz_base[:, 2] > BASE_SPACE[2][0]) & (d415_pc_xyz_base[:, 2] < BASE_SPACE[2][1])
        else:
        ...
        valid_mask = np.logical_and(clip_base_mask, np.logical_or(clip_object_mask, clip_bdft_mask))
        # TODO: hardcode to throw out hands
        # d415_pc_xyz_gripper = transform_pc(d415_pc_xyz_d415, np.linalg.inv(gripper_pose_d415))
        # valid_mask = np.logical_and(valid_mask, d415_pc_xyz_gripper[:, 2] > 0.)
        d415_pc_xyz_bdft = transform_pc(d415_pc_xyz_d415, np.linalg.inv(bdft_pose_d415))
        valid_mask = np.logical_and(valid_mask, d415_pc_xyz_bdft[:, 2] > 0.)
        ...
        if render_bdft_num != 0:
            bdft_pc_xyz_bdft_mesh = mesh2pc(os.path.join("objs", "only_peeler.obj"), num_points=render_bdft_num)
            ...
            d415_pc_xyz_d415_mesh = np.concatenate([d415_pc_xyz_d415_mesh, bdft_pc_xyz_d415_mesh], axis=0)
            d415_pc_rgb_mesh = np.concatenate([d415_pc_rgb_mesh, bdft_pc_rgb_mesh], axis=0)
        if voxel_size != 0:
            d415_pc_xyz_d415, d415_pc_rgb = voxelize(d415_pc_xyz_d415, d415_pc_rgb, voxel_size)
            d415_pc_xyz_d415_mesh, d415_pc_rgb_mesh = voxelize(d415_pc_xyz_d415_mesh, d415_pc_rgb_mesh, voxel_size)
        # if pc_num != -1:
        ...
        data_dict[stage_idx_map[stage_idx]]['d415_pc_xyzs_d415'].append(d415_pc_xyz_d415)
        data_dict[stage_idx_map[stage_idx]]['d415_pc_rgbs'].append(d415_pc_rgb)
        data_dict[stage_idx_map[stage_idx]]['d415_pc_xyzs_d415_mesh'].append(d415_pc_xyz_d415_mesh)
        data_dict[stage_idx_map[stage_idx]]['d415_pc_rgbs_mesh'].append(d415_pc_rgb_mesh)
        ...
        for stage_name in tqdm.tqdm(data_dict.keys()):
            with h5py.File(os.path.join(save_path, stage_name + '.hdf5'), 'w') as stage_hdf5:
                ...
                # 直接保存的字段
                fixed_xyzs_mesh = fix_pointcloud_list(data_dict[stage_name]['d415_pc_xyzs_d415'], pc_num)
                fixed_rgbs_mesh = fix_rgb_list(data_dict[stage_name]['d415_pc_rgbs'], pc_num)
                stage_hdf5_o_group.create_dataset('d415_pc_xyzs_d415', data=fixed_xyzs_mesh)
                stage_hdf5_o_group.create_dataset('d415_pc_rgbs', data=fixed_rgbs_mesh)

                # 先修正点云和颜色，使每帧点数固定为 pc_num
                fixed_xyzs_mesh = fix_pointcloud_list(data_dict[stage_name]['d415_pc_xyzs_d415_mesh'], pc_num)
                fixed_rgbs_mesh = fix_rgb_list(data_dict[stage_name]['d415_pc_rgbs_mesh'], pc_num)

                stage_hdf5_o_group.create_dataset('d415_pc_xyzs_d415_mesh', data=fixed_xyzs_mesh)
                stage_hdf5_o_group.create_dataset('d415_pc_rgbs_mesh', data=fixed_rgbs_mesh)

                # 其余字段正常保存
                stage_hdf5_o_group.create_dataset('gripper_xyzs_d415', data=np.array(data_dict[stage_name]['gripper_xyzs_d415']))
                stage_hdf5_o_group.create_dataset('gripper_quats_d415', data=np.array(data_dict[stage_name]['gripper_quats_d415']))
                stage_hdf5_o_group.create_dataset('bdft_xyzs_d415', data=np.array(data_dict[stage_name]['bdft_xyzs_d415']))
                stage_hdf5_o_group.create_dataset('bdft_quats_d415', data=np.array(data_dict[stage_name]['bdft_quats_d415']))
                stage_hdf5_o_group.create_dataset('bdft_fs_bdft', data=np.array(data_dict[stage_name]['bdft_fs_bdft']))
                stage_hdf5_o_group.create_dataset('bdft_ts_bdft', data=np.array(data_dict[stage_name]['bdft_ts_bdft']))
                stage_hdf5_o_group.create_dataset('bdft_fs_d415', data=np.array(data_dict[stage_name]['bdft_fs_d415']))
                stage_hdf5_o_group.create_dataset('bdft_ts_d415', data=np.array(data_dict[stage_name]['bdft_ts_d415']))
                stage_hdf5_o_group.create_dataset('angler_widths', data=np.array(data_dict[stage_name]['angler_widths']))

                # save attributes
                stage_hdf5_data_group.attrs['num_samples'] = len(data_dict[stage_name]['d415_pc_xyzs_d415'])
