...
class HandEyeCalibor(object):
    def __init__(
        self,
        marker_type: str = HANDEYE_MARKER_TYPE,
        ext_calib_params: dict = {
            "patter_size": CHESSBOARD_PATTERN_SIZE,
    ...
    def add_image_pose(
        self, img: np.ndarray, pose: np.ndarray, vis: bool = True
    ) -> bool:
        """
    ...
    def run(self) -> Optional[Tuple[np.ndarray, np.ndarray]]:
        ...
        if w2c is not None:
            R_b2w, t_b2w, R_g2c, t_g2c = cv2.calibrateRobotWorldHandEye(
                w2c[:, :3, :3], w2c[:, :3, 3], self.b2g[:, :3, :3], self.b2g[:, :3, 3]
            )
            b2w = np.eye(4)
            b2w[:3, :3] = R_b2w
            b2w[:3, 3] = t_b2w
...
if __name__ == "__main__":
    ...
    calibor = HandEyeCalibor(
        marker_type=args.marker_type, ext_calib_params=ext_calib_params
    )
