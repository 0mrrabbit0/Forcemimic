...
from r3kit.devices.camera.realsense.config import T265_ID, T265_STREAMS
from r3kit.utils.vis import draw_time, save_imgs


class T265(CameraBase):
    def __init__(
        self, id: Optional[str] = T265_ID, image: bool = True, name: str = "T265"
    ) -> None:
        ...
        pose_sensor = (
            self.pipeline.get_active_profile().get_device().first_pose_sensor()
        )
        self.pipeline.stop()
    ...
    def save_streaming(self, save_path: str, streaming_data: dict) -> None:
        assert (
            len(streaming_data["image"]["left"])
            == len(streaming_data["image"]["right"])
            == len(streaming_data["image"]["timestamp_ms"])
        )
        assert (
            len(streaming_data["pose"]["xyz"])
            == len(streaming_data["pose"]["quat"])
            == len(streaming_data["pose"]["timestamp_ms"])
        )
        if self._image:
            os.makedirs(os.path.join(save_path, "image"), exist_ok=True)
            np.save(
                os.path.join(save_path, "image", "timestamps.npy"),
                np.array(streaming_data["image"]["timestamp_ms"], dtype=float),
            )
            freq = len(streaming_data["image"]["timestamp_ms"]) / (
                streaming_data["image"]["timestamp_ms"][-1]
                - streaming_data["image"]["timestamp_ms"][0]
            )
            draw_time(
                streaming_data["image"]["timestamp_ms"],
                os.path.join(save_path, "image", f"freq_{freq}.png"),
            )
            os.makedirs(os.path.join(save_path, "image", "left"), exist_ok=True)
            os.makedirs(os.path.join(save_path, "image", "right"), exist_ok=True)
            save_imgs(
                os.path.join(save_path, "image", "left"),
                streaming_data["image"]["left"],
            )
            save_imgs(
                os.path.join(save_path, "image", "right"),
                streaming_data["image"]["right"],
            )
        os.makedirs(os.path.join(save_path, "pose"), exist_ok=True)
        np.save(
            os.path.join(save_path, "pose", "timestamps.npy"),
            np.array(streaming_data["pose"]["timestamp_ms"], dtype=float),
        )
        freq = len(streaming_data["pose"]["timestamp_ms"]) / (
            streaming_data["pose"]["timestamp_ms"][-1]
            - streaming_data["pose"]["timestamp_ms"][0]
        )
        draw_time(
            streaming_data["pose"]["timestamp_ms"],
            os.path.join(save_path, "pose", f"freq_{freq}.png"),
        )
        np.save(
            os.path.join(save_path, "pose", "xyz.npy"),
            np.array(streaming_data["pose"]["xyz"], dtype=float),
        )
        np.save(
            os.path.join(save_path, "pose", "quat.npy"),
            np.array(streaming_data["pose"]["quat"], dtype=float),
        )
    ...
    def callback(self, frame):
        ...
        if frame.is_frameset() and self._image:
            ...
            self.image_streaming_mutex.acquire()
            if (
                len(self.image_streaming_data["timestamp_ms"]) != 0
                and ts == self.image_streaming_data["timestamp_ms"][-1]
            ):
                pass
            else:
        ...
        if frame.is_pose_frame():
            ...
            quat = np.array(
                [
                    pose_data_.rotation.x,
                    pose_data_.rotation.y,
                    pose_data_.rotation.z,
                    pose_data_.rotation.w,
                ]
            )
            xyz = np.array(
                [
                    pose_data_.translation.x,
                    pose_data_.translation.y,
                    pose_data_.translation.z,
                ]
            )
            # ts = pose_frame.timestamp
            self.pose_streaming_mutex.acquire()
            if (
                len(self.pose_streaming_data["timestamp_ms"]) != 0
                and ts == self.pose_streaming_data["timestamp_ms"][-1]
            ):
                pass
            else:
