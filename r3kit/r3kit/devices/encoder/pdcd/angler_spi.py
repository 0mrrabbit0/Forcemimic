...
class Angler(EncoderBase):
    ...
    def __init__(self, id:str='/dev/ttyUSB0', index:int=1, fps:int=30,
                 baudrate:int=115200, gap:float=0.002, name:str='Angler') -> None:
    ...
    def start_streaming(self, callback:Optional[callable]=None) -> None:
        ...
        self.thread = Thread(target=partial(self._streaming_data, callback=callback),
                             daemon=True)
        self.thread.start()
