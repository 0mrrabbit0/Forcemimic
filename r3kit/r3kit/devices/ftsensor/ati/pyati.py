...
class PyATI(FTSensorBase):
    ...
    def save_streaming(self, save_path: str, streaming_data: dict) -> None:
        ...
        np.save(
            os.path.join(save_path, "ft.npy"),
            np.array(streaming_data["ft"], dtype=float),
        )
        draw_items(
            np.array(streaming_data["ft"], dtype=float),
            os.path.join(save_path, "ft.png"),
        )
    ...
    def raw2tare(
        ...
        f = raw_f - tare["f0"]
        f -= np.linalg.inv(pose) @ np.array([0.0, 0.0, -9.8 * tare["m"]])
        t = raw_t - tare["t0"]
        t -= np.linalg.inv(pose) @ np.cross(
            np.linalg.inv(pose) @ np.array(tare["c"]),
            np.array([0.0, 0.0, -9.8 * tare["m"]]),
        )
