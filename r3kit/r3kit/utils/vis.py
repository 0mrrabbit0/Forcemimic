...
def vis_pc(
    xyz: np.ndarray, rgb: Optional[np.ndarray] = None, show_frame: bool = True
) -> None:
    """
...
def rotation_vec2mat(vec: np.ndarray) -> np.ndarray:
    mat = np.zeros((3, 3))
    temp2 = np.cross(vec, np.array([1.0, 0.0, 0.0]))
    if np.linalg.norm(temp2) < 1e-3:
        temp1 = np.cross(np.array([0.0, 1.0, 0.0]), vec)
...
def save_img(
    ...
    if normalize:
        cv2.imwrite(
            os.path.join(path, f"{str(idx).zfill(16)}.{suffix}"),
            (
                (frame_list[idx] - frame_list[idx].min())
                / (frame_list[idx].max() - frame_list[idx].min())
                * 255
            ).astype(np.uint8),
        )
    else:
        cv2.imwrite(
            os.path.join(path, f"{str(idx).zfill(16)}.{suffix}"), frame_list[idx]
        )
...
def save_imgs(
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [
            executor.submit(save_img, idx, path, frame_list, suffix, normalize)
