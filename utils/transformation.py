...
def transform_pc(pc_camera: np.ndarray, c2w: np.ndarray) -> np.ndarray:
    ...
    pc_camera_hm = np.concatenate(
        [pc_camera, np.ones((pc_camera.shape[0], 1), dtype=pc_camera.dtype)], axis=-1
    )  # (N, 4)
...
def transform_dir(dir_camera: np.ndarray, c2w: np.ndarray) -> np.ndarray:
    ...
    dir_camera_hm = np.concatenate(
        [dir_camera, np.zeros((dir_camera.shape[0], 1), dtype=dir_camera.dtype)],
        axis=-1,
    )  # (N, 4)
